...
class NonNullTypeTest extends \PHPUnit_Framework_TestCase
{
    public function testNonNullType()
    {
        ...
        $nonNullOnString = new NonNullType(new StringType());
        $testArray       = ['a' => 'b'];

        $this->assertEquals($nonNullType->getName(), null, 'Empty non-null name');
        $this->assertEquals($nonNullType->getKind(), TypeKind::KIND_NON_NULL);
        $this->assertEquals($nonNullType, new NonNullType($stringType));
        ...
        $this->assertTrue($nonNullType->isValidValue($stringType->getName()));
